...
function ReserveList() {
  ...
  const handleSendPaymentReceiptV2 = (id) => {
    reserveService
      ...
      .catch((error) => {
        console.error(
          'Error al enviar el comprobante de pago (versión 2):',
          error,
        );
        ...
        <TableBody>
          {reserves.map((reserve) => (
            <TableRow key={reserve.id}>
              ...
              <TableCell sx={{ color: 'var(--text-color)' }}>
                {reserve.reserves_users.length > 0
                  ? reserve.reserves_users[0].name
                  : 'Sin cliente'}
              </TableCell>
              <TableCell sx={{ color: 'var(--text-color)' }}>
                {formatDate(reserve.reserveday)}
              </TableCell>
              <TableCell align="center" sx={{ color: 'var(--text-color)' }}>
                {reserve.reserves_users.length}
              </TableCell>
              <TableCell align="center" sx={{ color: 'var(--text-color)' }}>
                {typeof reserve.final_price === 'number'
                  ? reserve.final_price.toLocaleString('es-CL', {
                    style: 'currency',
                    currency: 'CLP',
                  })
                  : 'N/A'}
