...
do {
    ...
    # Verificar si Eureka puede conectarse al Config Service
    ...
    if ($connectionErrors) {
        Write-Host "Error: Eureka no pudo conectarse al Config Service." -ForegroundColor Red
        ...
        kubectl wait --for=condition=ready pod -l app=eureka-service-deployment --timeout=300s
        if ($LASTEXITCODE -ne 0) {
            Write-Host "Error: eureka-service sigue sin estar listo después del reinicio." -ForegroundColor Red
            Pause
            return
        }
    ...
    Write-Host "📊 Eureka: http://localhost:30761" -ForegroundColor Cyan
    Write-Host ""
